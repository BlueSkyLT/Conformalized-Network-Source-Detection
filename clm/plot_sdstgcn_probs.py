...
def plot_probability_barchart(probs, source_nodes, title, output_path=None, 
                               top_k=None, sort_by_prob=False):
    """
    Plot bar chart of source probabilities for all nodes.
    ...
    Args:
        ...
        top_k: if provided, only show top K nodes by probability
    ...
    # Create color array: green for GT sources, blue for others
    source_set = set(source_nodes)
    colors = ['green' if n in source_set else 'steelblue' for n in nodes]
    
    # Determine figure size based on number of nodes
    width = max(10, min(20, len(nodes) * 0.15))
    ...
    legend_elements = [
        Patch(facecolor='green', edgecolor='black', label='Ground Truth Source'),
...
def main():
    ...
    parser.add_argument('--top_k', type=int, default=None, 
                        help='Only show top K nodes by probability (default: all)')
    parser.add_argument('--sort', action='store_true', 
                        help='Sort nodes by probability (descending)')
    parser.add_argument('--all_nodes', action='store_true',
                        help='Show all nodes even if there are many (default: auto-limit)')
    
    args = parser.parse_args()
    
    # Load data
    predictions, ground_truths = load_res_pickle(args.res_path)
    ...
    # Determine top_k
    top_k = args.top_k
    if top_k is None and not args.all_nodes and n_nodes > 100:
        top_k = 100
        print(f"\n  Auto-limiting to top {top_k} nodes. Use --all_nodes to show all.")
    ...
    # Plot
    plot_probability_barchart(
        probs, 
        source_nodes, 
        title, 
        output_path=args.output,
        top_k=top_k,
        sort_by_prob=args.sort or (top_k is not None)
    )
